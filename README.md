# humanoid_unitree_go1 — Unitree Go1

## 项目简介

Unitree Go1 四足机器人应用包，12 自由度（FR/FL/RR/RL 各 3：hip、thigh、calf）。包含该机型专属的 YAML 配置、MuJoCo 仿真资源、RL 策略模型及启动脚本，通用控制逻辑见 `humanoid_common` 仓库。
...
lunch k3-com260-kit-humanoid-unitree-go1
m
```
...
```bash
run_driver_go1.sh    # 终端1（PC，x86_64）
run_control_go1.sh   # 终端2（K3 板卡）
run_hmi_go1.sh       # 终端3（K3 板卡）
```

**sim2sim（双终端）**：

```bash
run_driver_go1.sh    # 终端1（PC）
run_sim2sim_go1.sh   # 终端2（K3 板卡）
```
...
| 现象 | 处理 |
| --- | --- |
| `[PolicyConfigLoader] ONNX 模型文件不存在` | 运行 `download_models_go1.sh` 下载模型后重试 |
| 进程启动后通信无数据 | 检查 `config/go1.yaml` 中 `driver_ip` / `control_ip` 是否正确，确认两机可互相 ping 通 |
...
| 版本 | 说明 |
| --- | --- |
| 0.1.0 | 初始版本，支持 Go1 12-DOF FSM 与 sim2sim 仿真 |
